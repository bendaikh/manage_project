<template>
  <div class="max-w-6xl mx-auto p-6">
    <h2 class="text-2xl font-bold mb-6">Sellers Invoices</h2>
      ...
      <table class="min-w-full bg-white rounded-lg shadow">
        ...
        <tbody>
          ...
          <tr v-if="invoices.length === 0">
            <td colspan="5" class="text-center py-6 text-gray-400">No invoices found.</td>
          </tr>
        </tbody>
      </table>
      <!-- Pagination -->
...
const fetchInvoices = async () => {
  const res = await fetch(`/seller-invoices?page=${currentPage.value}&per_page=${perPage.value}`)
  if (res.ok) {
...
const downloadInvoice = async (invoice) => {
  try {
    const response = await fetch(`/seller-invoices/${invoice.id}/download`)

    if (response.ok) {
      window.open(`/seller-invoices/${invoice.id}/download`, '_blank')
    } else {
